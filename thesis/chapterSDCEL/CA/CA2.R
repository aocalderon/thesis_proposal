...
bar1 = ggplot(data = data, aes(x = partitions, y = time, fill="blue")) +
  ...
  theme_bw() +
  guides(fill = F)
bar2 = ggplot(data = cgal, aes(x = partitions, y = time, fill="blue")) +
  geom_bar(stat="identity", width = 0.75) + 
  facet_grid(~method) +
  scale_y_continuous(breaks = seq(0, 800, 200)) + coord_cartesian(ylim = c(0, 800)) + 
  labs(x="", y="", title="") +
  theme_bw() +
  guides(fill = F)
p = plot_grid(bar2, bar1, align = "h", ncol = 2, rel_widths = c(1.5/10, 7/10))
...
q = ggplot(data = data %>% filter(capacity %in% sample), aes(x = partitions, y = time, fill="blue")) +
