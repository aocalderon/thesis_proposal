...
p = ggplot(data3, aes(x = partitions, y = time, fill = stage)) + 
  ...
  labs(x="Number of cells", y="Time [s]", caption = "(a)") +
...
q = ggplot(data3, aes(x = partitions, y = time, fill = stage)) + 
  geom_col(width = 0.7) + 
  scale_fill_discrete(labels=c('Layer A', 'Layer B', 'Overlay')) +
  labs(x="Number of cells", y="Time [s]", caption = "(b)") +
  guides(fill=guide_legend(title="Stages")) +
  theme_bw()  + 
  theme(legend.position="top", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        plot.caption = element_text(hjust = 0.5, size = 12))
...
ggsave("Performance.pdf", width = 8, height = 4)